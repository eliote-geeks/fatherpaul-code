...
impl CodexMessageProcessor {
    ...
    async fn login_api_key_common(
        ...
        if matches!(
            ...
            return Err(JSONRPCErrorError {
                code: INVALID_REQUEST_ERROR_CODE,
                message: "API key login is disabled. Use your Father Paul account instead."
                    .to_string(),
                data: None,
            });
        }
    ...
    async fn login_chatgpt_common(
        ...
        if matches!(config.forced_login_method, Some(ForcedLoginMethod::Api)) {
            return Err(JSONRPCErrorError {
                code: INVALID_REQUEST_ERROR_CODE,
                message: "Managed account login is disabled. Use API key login instead."
                    .to_string(),
                data: None,
            });
        }
    ...
    async fn login_chatgpt_auth_tokens(
        ...
        if matches!(
            ...
            let error = JSONRPCErrorError {
                code: INVALID_REQUEST_ERROR_CODE,
                message: "External managed account auth is disabled. Use API key login instead."
                    .to_string(),
                data: None,
            };
            self.outgoing.send_error(request_id, error).await;
