...
#[clap(
    ...
    // `fatherpaul-code-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `fatherpaul-code` command name that users run.
    bin_name = "fatherpaul-code",
    override_usage = "fatherpaul-code [OPTIONS] [PROMPT]\n       fatherpaul-code [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run FatherPaul Code non-interactively.
    ...
    /// Manage external MCP servers for FatherPaul Code.
    Mcp(McpCli),

    /// Start FatherPaul Code as an MCP server (stdio).
    McpServer,

    /// [experimental] Run the app server or related tooling.
    AppServer(AppServerCommand),

    /// Launch the FatherPaul Code desktop app (downloads the macOS installer if missing).
    #[cfg(target_os = "macos")]
    ...
    /// Run commands within a FatherPaul Code-provided sandbox.
    Sandbox(SandboxArgs),
    ...
    /// Apply the latest diff produced by FatherPaul Code as a `git apply` to your local working tree.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv FATHERPAUL_API_KEY | fatherpaul-code login --with-api-key`)"
    )]
    with_api_key: bool,
...
struct AppServerCommand {
    ...
    /// ```
    ///
    /// See the FatherPaul Code configuration guide for more details.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for FatherPaul Code tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating FatherPaul Code via `{cmd_str}`...");
    ...
    println!("\nUpdate ran successfully. Please restart FatherPaul Code.");
    Ok(())
}
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv FATHERPAUL_API_KEY | fatherpaul-code login --with-api-key`."
                        );
                        std::process::exit(1);
                        ...
                        eprintln!(
                            "FatherPaul Code uses Father Paul API keys. Export FATHERPAUL_API_KEY or pipe it via stdin, e.g. `printenv FATHERPAUL_API_KEY | fatherpaul-code login --with-api-key`."
                        );
                        std::process::exit(1);
                    }
                }
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". FatherPaul Code may not work correctly in this terminal."
        );
