...
impl ChatWidget {
    ...
    fn plugin_install_auth_popup_params(&self) -> Option<SelectionViewParams> {
        ...
        let status_label = if is_installed {
            ...
            "Install the required apps in Father Paul Portal to continue:"
        };
        ...
        if let Some(install_url) = app.install_url.clone() {
            let install_label = if is_installed {
                "Manage in Father Paul Portal"
            } else {
                "Install in Father Paul Portal"
            };
            items.push(SelectionItem {
                name: install_label.to_string(),
                description: Some("Open the Father Paul app management page".to_string()),
            ...
            items.push(SelectionItem {
                name: "Father Paul app link unavailable".to_string(),
    ...
    fn plugins_loading_popup_params(&self) -> SelectionViewParams {
        SelectionViewParams {
            ...
            header: Box::new(DelayedLoadingHeader::new(
                ...
                Some("This first pass shows the Father Paul app marketplace only.".to_string()),
            )),
