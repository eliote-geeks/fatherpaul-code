...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with your Father Paul account to use FatherPaul Code under your active plan"
                    .into(),
            ]),
            Line::from(vec![
                "  ".into(),
                "or connect a Father Paul API key for usage-based billing".into(),
            ]),
            "".into(),
        ];
        ...
        let chatgpt_description = if !self.is_chatgpt_login_allowed() {
            ...
            "Usage included with your Father Paul plan when supported by your workspace"
        };
            ...
            match option {
                SignInOption::ChatGpt => {
                    lines.extend(create_mode_item(
                        idx,
                        option,
                        "Sign in with Father Paul account",
                        chatgpt_description,
                    ));
                }
                SignInOption::DeviceCode => {
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  API key login is disabled by this workspace. Sign in with your Father Paul account to continue."
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Signed in with your Father Paul account"
                .fg(Color::Green)
                .into(),
            "".into(),
            "  Before you start:".into(),
            "".into(),
            "  Decide how much autonomy you want to grant FatherPaul Code".into(),
            Line::from(vec![
                "  For more details see the ".into(),
                "\u{1b}]8;;https://fatherpaulai.com/use-cases.html\u{7}Father Paul documentation\u{1b}]8;;\u{7}".underlined(),
            ])
            .dim(),
            "".into(),
            "  FatherPaul Code can make mistakes".into(),
            "  Review the code it writes and commands it runs".dim().into(),
            "".into(),
            "  Powered by your Father Paul account".into(),
            Line::from(vec![
                "  Uses your active plan, local configuration, and API access settings.".into(),
            ])
            .dim(),
            "".into(),
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  FatherPaul Code will use usage-based billing with your API key.".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "Use your own Father Paul API key for usage-based billing".bold(),
            ]),
            "".into(),
        ...
        if state.prepopulated_from_env {
            intro_lines.push("  Detected FATHERPAUL_API_KEY environment variable.".into());
            intro_lines.push(
