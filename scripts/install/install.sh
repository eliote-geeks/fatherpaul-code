...
download_file() {
  ...
  echo "curl or wget is required to install FatherPaul Code." >&2
  exit 1
}
...
resolve_version() {
  ...
  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest FatherPaul Code release version." >&2
    exit 1
  fi
...
step "$install_mode FatherPaul Code"
...
step "Downloading FatherPaul Code"
